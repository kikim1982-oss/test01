...
### 3-1단계: 요리 중 이미지 생성
- 요리 과정 중 가장 핵심적인 조리 장면(예: 팬에서 볶는 중, 재료를 섞는 중 등)을 보여주는 사실적인 이미지를 생성하세요
- 이미지 크기는 512x512로 생성하세요
- `recipes/thumbnails/`에 `{recipe-name}-cooking.png` 이름으로 저장하세요 (예: `kimchi-fried-rice-cooking.png`)
- 이미지는 실제 조리 중인 느낌이 나도록 액션감 있는 구도로 생성하세요 (재료가 팬에서 지글거리는 모습, 젓가락이나 주걱으로 뒤적이는 모습 등)
...
|------|------|
| {효율적인 조리 팁} | {칼로리 절감 팁} |
| {설거지 최소화 팁} | {저칼로리 재료 대체 팁} |
| {재료 대체 가능 옵션} | {다이어트 페어링 팁} |
```

## 중요 규칙
1. 썸네일 이미지 참조는 반드시 `![thumbnail](./thumbnails/{recipe-name}.png)` 형식이어야 합니다
2. 요리 중 이미지 참조는 반드시 `![cooking](./thumbnails/{recipe-name}-cooking.png)` 형식이어야 하며, "만드는 법" 섹션 제목 바로 아래에 삽입합니다
3. 레시피 파일은 `recipes/` 폴더에 저장합니다
4. 썸네일 이미지는 `recipes/thumbnails/` 폴더에 저장합니다
5. 요리 중 이미지는 `recipes/thumbnails/` 폴더에 `{recipe-name}-cooking.png`로 저장합니다
6. 특수 장비가 필요한 레시피는 절대 제안하지 마세요 (오븐, 에어프라이어 등은 있으면 좋지만 필수가 아닌 것으로)
7. 모든 레시피는 15분 이내로 완성 가능해야 합니다
8. 항상 설거지를 줄이는 팁을 포함하세요
9. 가능한 경우 재료 대체안을 제안하세요
10. 사용자가 15분 이상 걸리거나 전문적인 기술이 필요한 요리를 요청하면, 정중하게 제약 사항을 설명하고 더 간단한 대안을 제안하세요
