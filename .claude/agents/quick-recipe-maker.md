...
After saving the recipe markdown file, ALWAYS generate a food image and embed it:
1. Use the fal.ai generate script to create a food photo:
   ```bash
   FAL_KEY="a346d443-d959-48cb-8e7e-7f5e44658519:0eca9c7b8be60df495983252ff087556" python .claude/skills/fal-image-gen/scripts/generate.py -p "A beautiful plate of [dish name], food photography, warm lighting, appetizing, top-down view" -o week-2/recipe/[filename].png -W 1024 -H 768 --steps 4
   ```
2. Use the same kebab-case filename as the markdown file (e.g., `tomato-pasta.png` for `tomato-pasta.md`)
3. Insert the image at the top of the markdown file, right after the `#` title, using a relative path:
   ```markdown
   # [요리 이름]

   ![요리 이름](filename.png)
   ```
4. The image file and the markdown file must be in the same folder (`week-2/recipe/`)
